...
本模板修改自<a href="https://github.com/dryangyq/ouc-slide-latex-template" target="_blank">中国海洋大学幻灯片 LaTeX 模板</a>，并参考既有高校 Beamer 模板结构进行了燕山大学适配。

模板主要使用燕山大学形象识别系统中的标准色与辅助色，包括燕大蓝、亮蓝色、浅蓝色、辅助红、中性灰、金色、银色。
...
1. slides.tex：主文件，编写的演示文稿内容主要存放在该文件当中。
2. ysuslide.sty：模板的样式文件。
3. font:字体文件夹，所有编译需要用到的字体。默认字体使用的是华为鸿蒙字体。
4. theme:主题文件夹。
    1. beamercolorthemeYSUSlide.sty：颜色定义文件。
    2. beamerfontthemeYSUSlide.sty：字体定义文件。
    3. beamerinnerthemeYSUSlide.sty：内部元素定义文件。
    4. beamerouterthemeYSUSlide.sty：整体布局定义文件。
    5. beamerthemeYSUSlide.sty：主题定义文件。
...
## 字体
燕山大学 VIS 手册中的中文专用印刷字体包括方正大黑、方正兰亭黑、方正细黑、方正宋体及宋体/仿宋/楷体 GB2312，英文专用印刷字体包括 Helvetica、Times New Roman、Arial 等。

模板面向屏幕演示，默认使用随模板分发的<a href="https://developer.harmonyos.com/cn/docs/design/font-0000001157868583" target="_blank">华为鸿蒙字体 (HarmonyOS Sans)</a>作为方正黑体与 Helvetica 类无衬线字体的可移植替代。该字体可以免费商用，且能保证跨平台编译一致。

如果需要严格按纸质印刷规范制作，可下载并授权使用方正兰亭黑、方正细黑等字体，放入 fonts 文件夹，并在`beamerfontthemeYSUSlide.sty`文件中修改。

## 颜色
![color](figs/slides_9.png)

## 颜色等级
为了显示更丰富的色彩，定义的燕山大学视觉系统标准色，每一种颜色，都有九个等级，等级越低，颜色越浅。例如：对于燕大蓝，其等级颜色从 `ysublue90` 到 `ysublue20` 不等，分成 9 个等级。
